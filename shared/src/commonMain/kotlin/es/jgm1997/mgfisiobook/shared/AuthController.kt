package es.jgm1997.mgfisiobook.shared

import es.jgm1997.mgfisiobook.core.repositories.AuthRepository
import es.jgm1997.mgfisiobook.core.auth.AuthStorage
import es.jgm1997.mgfisiobook.shared.auth.AuthResult
import es.jgm1997.mgfisiobook.shared.auth.JwtParser
import es.jgm1997.mgfisiobook.shared.usecase.RegisterDeviceUseCase
import es.jgm1997.mgfisiobook.shared.push.PushTokenProvider


class AuthController(
    private val authRepository: AuthRepository,
    private val pushTokenProvider: PushTokenProvider,
    private val registerDeviceUseCase: RegisterDeviceUseCase
) {
    suspend fun login(email: String, password: String): AuthResult {
        return try {
            val token = authRepository.login(email, password)
            AuthStorage.saveToken(token)
            val access = token.accessToken ?: return AuthResult.Error("Token sin access_token")
            val userId = JwtParser.extract(access, "sub")
                ?: return AuthResult.Error("No se pudo extraer el userId del JWT")
            val pushToken = pushTokenProvider.getPushToken()
            if (pushToken != null) registerDeviceUseCase(
                pushToken,
                pushTokenProvider.platform(),
                pushTokenProvider.appVersion(),
                userId.toString()
            )
            AuthResult.Success(token, userId.toString())
        } catch (e: Exception) {
            AuthResult.Error("Login failed", e)
        }
    }
}